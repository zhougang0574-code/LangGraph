"""
第十四课：create_agent —— 一行代码建好 ReAct Agent
===================================================
第八课手动搭了 ReAct Agent：
...
# ── 2. 一行创建 ReAct Agent ───────────────────────────────────────────────
# 对比第八课需要 7 步，现在只需要这一行：
graph = create_agent(llm, tools)

# 需要记忆功能时，传入 checkpointer（和第九课一样）：
...
# ── 说明 ────────────────────────────────────────────────────────────────────
"""
create_agent(model, tools) 内部等价于第八课的：
...
可选参数：
  checkpointer  → 传入 MemorySaver() 等，启用多轮记忆（第九课）
  system_prompt → 系统提示，字符串或 SystemMessage（第二课）

什么时候用 create_agent，什么时候手动搭：
  需要工具调用的标准 ReAct 循环 → create_agent，省事
  需要自定义路由、额外节点、非标准逻辑 → 手动搭（第八课的方式）
"""
