"""
【05 持久化与记忆 / 01】对话记忆 —— 两次 invoke() 之间保留历史
================================================
〔04_工具与Agent/02〕的 ReAct Agent 每次 invoke() 都是全新的开始，
前一次的对话内容不会保留。
...
# ── 1. State：和〔02_状态State/02〕一样 ────────────────────────────────────────────────
class State(TypedDict):
    messages: Annotated[list[BaseMessage], add_messages]


# ── 2. 节点：和〔02_状态State/02〕一样 ─────────────────────────────────────────────────
def llm_node(state: State) -> dict:
    response = llm.invoke(state["messages"])
    return {"messages": [response]}


# ── 3. Graph：结构和〔02_状态State/02〕一样，只有 compile() 多了一个参数 ──────────────
