"""
【02 状态State / 01】消息格式 —— HumanMessage 与 SystemMessage
=================================================
与〔01_基础/01〕的区别：只改了 LLM 的调用方式，Graph 结构完全不变。

〔01_基础/01〕：llm.invoke("字符串")          ← 最简单，但不够灵活
〔02_状态State/01〕：llm.invoke([HumanMessage(...)]) ← 标准消息格式
...
# ── 1. State：比〔01_基础/01〕多了一个 role 字段 ───────────────────────────────────
...
# ── 2. 节点：只改了 LLM 的调用方式，其余和〔01_基础/01〕一样 ─────────────────────
...
★ 对比〔01_基础/01〕：
  ...
  〔01_基础/01〕：llm.invoke("什么是人工智能？")
          → LLM 按默认风格回答

  〔02_状态State/01〕：llm.invoke([
