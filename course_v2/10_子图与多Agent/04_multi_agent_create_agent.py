"""
【10 子图与多Agent / 04】Multi-Agent 进阶 —— 用 create_agent 替代手写子图
========================================================================
和 〔10_子图与多Agent/03〕 同样是 Supervisor 多 Agent，只换一处实现方式：
  〔10_子图与多Agent/03〕 的数学子 Agent：手写 MathState + ReAct 子图（prepare/agent/tools/finish）
  本课：               直接用 〔05_工具与Agent/03〕 的 create_agent 一行建好子 Agent

对比：
  手写子图：自定义 State → 写各节点 → 搭 builder → compile()
  create_agent：create_agent(llm, tools) 一行搞定，外面再包一层函数节点做 State 转换
...
# ── 说明 ────────────────────────────────────────────────────────────────────
"""
本课 vs 〔10_子图与多Agent/03〕：

  〔10_子图与多Agent/03〕  数学子 Agent = 手写子图（自定义 MathState + ToolNode 循环）
  本课                   数学子 Agent = create_agent(llm, tools) 一行

什么时候用哪种：
  create_agent  → 标准 ReAct 子 Agent，省事，首选
  手写子图      → 子 Agent 内部需要自定义节点 / 非标准路由时

注意：create_agent 内部是 MessagesState，外层是 task/result 自定义 State，
     所以函数节点里要做一次「task → messages」和「messages[-1] → result」的转换。
"""
